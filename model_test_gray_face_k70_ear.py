...
# 사용자 정의 모델 로드
model_path = 'v3l_adam_lr001_aug.h5'  # 저장된 모델의 경로
...
while cap.isOpened():
    ...
    left_eye_indices = [i - 1 for i in left_eye_indices]  # 0부터 시작하는 인덱스로 조정
    right_eye_indices = [42, 43, 44, 45, 46, 47]
    right_eye_indices = [i - 1 for i in right_eye_indices]  # 0부터 시작하는 인덱스로 조정
    ...
    try:
        ...
        print(f"[EAR] Left EAR: {left_EAR:.3f}, Right EAR: {right_EAR:.3f}, Average EAR: {ear:.3f}")
    except Exception as e:
        print(f"[EAR 계산 오류] {e}")
        continue
